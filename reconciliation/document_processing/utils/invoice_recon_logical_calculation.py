        ...
        async with semaphore:
            try:
                ...
                invoice_items = await sync_to_async(list)(
                    InvoiceItemData.objects.filter(invoice_data=invoice.id)
                )
                ...
                # Precompute financial comparison in Python and add to invoice_json
                financial_comparison = self.calculate_variance_logically(
                    invoice_json.get("invoice_financial_data", {}),
                    grn_json.get("grn_summary", {})
                )
                invoice_json["precomputed_financial_comparison"] = financial_comparison
    ...
    async def _convert_invoice_to_complete_json(self, invoice: InvoiceData, invoice_items: List[InvoiceItemData]) -> Dict[str, Any]:
        ...
        for item in invoice_items:
            invoice_json["invoice_line_items"].append({
                ...
                "sgt_rate": str(item.sgst_rate or 0),
                "sgt_amount": str(item.sgst_amount or 0),
    ...
    async def _convert_grn_to_complete_json(self, grn_items: List[ItemWiseGrn]) -> Dict[str, Any]:
        ...
        grn_json = {
            "grn_summary": {
                ...
                "sgt_sum": str(sum(Decimal(str(item.sgst_tax_amount or 0)) for item in grn_items)),
...
"""
    ...
    async def _create_reconciliation_from_llm_analysis(self, invoice: InvoiceData, grn_items: List[ItemWiseGrn],
        ...
        if llm_analysis and llm_analysis.get('success'):
            ...
            def get_variance_amount(comp_data, key):
                section = financial_comp.get(key, {})
                return section.get('variance_amount', 0)
            
            subtotal_var = get_variance_amount(financial_comp, 'subtotal_comparison')
            cgst_var = get_variance_amount(financial_comp, 'cgst_comparison')
            sgst_var = get_variance_amount(financial_comp, 'sgst_comparison')
            igst_var = get_variance_amount(financial_comp, 'igst_comparison')
            total_var = get_variance_amount(financial_comp, 'total_comparison')
            ...
            subtotal_var = invoice_subtotal - grn_subtotal
            cgst_var = invoice_cgst - grn_cgst
            sgst_var = invoice_sgst - grn_sgst
            igst_var = invoice_igst - grn_igst
            total_var = invoice_total - grn_total
            
            grn_summary = grn_json.get('grn_summary', {})
        ...
        invoice_subtotal_final = float(invoice.invoice_value_without_gst or 0)
        invoice_cgst_final = float(invoice.cgst_amount or 0)
        invoice_sgst_final = float(invoice.sgst_amount or 0)
        invoice_igst_final = float(invoice.igst_amount or 0)
        invoice_total_final = float(invoice.invoice_total_post_gst or 0)

        grn_subtotal_final = float(sum(Decimal(str(item.subtotal or 0)) for item in grn_items))
        grn_cgst_final = float(sum(Decimal(str(item.cgst_tax_amount or 0)) for item in grn_items))
        grn_sgst_final = float(sum(Decimal(str(item.sgst_tax_amount or 0)) for item in grn_items))
        grn_igst_final = float(sum(Decimal(str(item.igst_tax_amount or 0)) for item in grn_items))
        grn_total_final = float(sum(Decimal(str(item.total or 0)) for item in grn_items))
        ...
        reconciliation_data = {
            ...
            'invoice_subtotal': invoice_subtotal_final,
            'invoice_cgst': invoice_cgst_final,
            'invoice_sgst': invoice_sgst_final,
            'invoice_igst': invoice_igst_final,
            'invoice_total': invoice_total_final,

            # GRN aggregated amounts (from grn_json summary)
            'grn_subtotal': grn_subtotal_final,
            'grn_cgst': grn_cgst_final,
            'grn_sgst': grn_sgst_final,
            'grn_igst': grn_igst_final,
            'grn_total': grn_total_final,
    ...
    @staticmethod
    def calculate_variance_logically(invoice_financial: Dict[str, Any], grn_summary: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        ...
        def get_variance(invoice_val, grn_val):
            invoice = safe_decimal(invoice_val)
            grn = safe_decimal(grn_val)
            variance = invoice - grn
            variance_pct = (variance / invoice * 100) if invoice != 0 else Decimal("0")
            return {
                "variance_amount": float(round(variance, 2)),
                "variance_pct": float(round(variance_pct, 2)),
                "invoice_value": float(round(invoice, 2)),
                "grn_value": float(round(grn, 2)),
                "match": abs(variance) < 0.01
            }
        
        return {
            "subtotal_comparison": get_variance(invoice_financial.get("invoice_value_without_gst"), grn_summary.get("subtotal_sum")),
            "cgst_comparison": get_variance(invoice_financial.get("cgst_amount"), grn_summary.get("cgst_sum")),
            "sgt_comparison": get_variance(invoice_financial.get("sgt_amount"), grn_summary.get("sgt_sum")),
            "igst_comparison": get_variance(invoice_financial.get("igst_amount"), grn_summary.get("igst_sum")),
            "total_comparison": get_variance(invoice_financial.get("invoice_total_post_gst"), grn_summary.get("total_amount_sum")),
        }
